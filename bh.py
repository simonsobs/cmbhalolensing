...
for i,task in enumerate(my_tasks):
    ...
    if i==0:
        ...
        feed_dict['pc_T_T'] = 1.
        # h = qe.HardenedTT(shape,wcs,feed_dict,xmask=xmask,ymask=ymask,kmask=kmask,Al=None,estimator='hdv',hardening='src')
    ...
    # Deconvolve beam
    akmap1 = enmap.fft(aobs1,normalize='phys') / abeam
    akmap2 = enmap.fft(aobs2,normalize='phys') / abeam

    assert np.all(np.isfinite(akmap1))
    assert np.all(np.isfinite(akmap2))
    ...
    aobs3 = fit_and_subtract(aobs2)

    akmap3 = enmap.fft(aobs3,normalize='phys') / abeam
    feed_dict['X'] = akmap1
    feed_dict['Y'] = akmap3

    brkmap = q.reconstruct(feed_dict,xname='X_l1', yname='Y_l2', physical_units=True)
