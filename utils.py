    ...
    if cat_type=='hilton_dr5':
        ...
        if is_meanfield:
            ...
            for i, file_i in enumerate(desi_files):
                desi_data = catalogs.load_fits(file_i,["ra", "dec"])
                ras=np.append(ras,desi_data["ra"])
                decs=np.append(decs,desi_data["dec"])
            zs=np.zeros(len(ras))
        else:
            desi_file = paths.desi_lrg_data + "LRG_full_HPmapcut.dat.fits"
            desi_data = catalogs.load_fits(desi_file,["ra", "dec", "z_not4clus"])
            ras = desi_data["ra"]
            decs = desi_data["dec"]
...
def postprocess(stack_path,mf_path,save_name=None,ignore_param=False,args=None,ignore_last=None):
    ...
    if not(save_name is None):
        ...
        np.savetxt(f'{save_dir}/{save_name}_all_profiles.txt', profs)
