...
act_sim_path = "/home3/eunseong/nemo-sims/nemo-sim-kit5/sim-kit_NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles/NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles"
output_path = "/home3/eunseong/cmbhalolensing/wsky_outs"
mat_path = "/home1/mathm/repos/cmbhalolensing"
test_path = "/home3/eunseong/nemo-sims/nemo-sim-kit5/sim-kit_NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles/sim-maps"
...
print(" ::: test:", test, "/ mean_field:", mean_field, "/ inpainting:", inpainting, "/ true_map:", true_map)
...
def load_beam(freq):
    if freq=="f150": fname = f"{my_path}/act_planck_dr5.01_s08s18_f150_daynight_beam.txt"  # original beam (beam used by Matt to create the sims)
    elif freq=="f090": fname = f"{my_path}/act_planck_dr5.01_s08s18_f090_daynight_beam.txt"    
...
if mean_field == True: 
    ...
    if which_cat == "halo":
        ...
        cat = f"{my_path}/websky_ACT_cmb_tsz_randoms_dlensed_snr5p5.txt"
    elif which_cat == "act_tsz_ksz_cib": 
        cat = f"{my_path}/websky_ACT_cmb_tsz_ksz_cib_randoms_updated.txt" # corresponding to a new act sim (202404)
    ...
    if which_cat == "halo":
        ...
        cat = f"{act_sim_path}/NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles_mass.fits" 
        hdu = fits.open(cat)
        ras = hdu[1].data["RADeg"]
        decs = hdu[1].data["DECDeg"]
        # mass = hdu[1].data["M500c"] # 1e14 Msun
        mass = hdu[1].data["M200m"] # 1e14 Msun
        snr = hdu[1].data["SNR"]
        ...
        print(" ::: min SNR of the sample is ", snr.min()) # 4.01
        print(" ::: mean mass of the sample is ", mass.mean()) # 7.63

        # np.savetxt(f"{my_path}/websky_ACT_cmb_tsz_lensed_alm.txt", np.c_[ras, decs, zs, mass]) # just to check
        # sys.exit()

    elif which_cat == "act_tsz_ksz_cib":

        # cat = f"{act_sim_path}/ACTSim_CMB-T_cmb-tsz-ksz-cib_MFMF_pass2_mass.fits" # 7943
        cat = f"{act_sim_path}/NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles/NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles_mass.fits" # 4514
        hdu = fits.open(cat)
        # print(hdu[1].columns)
        ras = hdu[1].data["RADeg"]
        decs = hdu[1].data["DECDeg"]
...
if which_cat == "act_tsz":
    if freq_sz == 150:
        imap = f"{act_sim_path}/clusterModelMaps/clusterModelMap_f150.fits" 
        # imap = f"{test_path}/simMap_f150_signalOnly.fits" # true tsz cluster model pasted into the cmb map when creating catalogue
    elif freq_sz == 90: 
        imap = f"{act_sim_path}/clusterModelMap_f090.fits"  
elif which_cat == "act_tsz_ksz_cib":
    if freq_sz == 150:
        # imap = f"{act_sim_path}/model_MFMF_pass2_cmb-tsz-ksz-cib_f150.fits"
        imap = f"{act_sim_path}/NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles/clusterModelMaps/clusterModelMap_f150.fits"
    elif freq_sz == 90:
        # imap = f"{act_sim_path}/model_MFMF_pass2_cmb-tsz-ksz-cib_f090.fits"
        imap = f"{act_sim_path}/NemoWebSky_CustomLensedCMB_tenToA0Tuned_ACT-DR5-2Pass_2degTiles/clusterModelMaps/clusterModelMap_f090.fits"
        
modelmap = enmap.read_map(imap)
shape, wcs = modelmap.shape, modelmap.wcs  
print(" ::: reading tsz cluster model image map at %d GHz:" %freq_sz, imap)
print("modelmap", np.shape(modelmap), wcs) # (10320, 43200) car:{cdelt:[-0.008333,0.008333],crval:[0,0],crpix:[21601.00,7561.00]}
...
# print("lmap", np.shape(lmap), lmap.wcs) # (10320, 43200)
...
# b_l_tsz_map = f"{my_path}/beam_convolved_actfoot_dlensed_tsz2048_map.fits" 

# enmap.write_map(b_l_tsz_map, l_tsz_map)
# print(" ::: beam-convolved maps are saved!")  
...
r_lmap = f"{my_path}/reproj_actfoot_dlensed.fits" # hich accuracy lensed map from Mat
...
# b_lmap = f"{my_path}/beam_convolved_actfoot_lensed_alm_map.fits" 
# b_l_tsz_map = f"{my_path}/beam_convolved_actfoot_lensed_alm_tsz_map.fits" 
# b_l_ksz_cib_map = f"{my_path}/beam_convolved_actfoot_lensed_alm_ksz_cib_map.fits"
# b_l_tsz_ksz_cib_map = f"{my_path}/beam_convolved_actfoot_lensed_alm_tsz_ksz_cib_map.fits"
b_lmap = f"{my_path}/beam_convolved_actfoot_dlensed_map.fits" # hich accuracy lensed map from Mat
b_l_tsz_map = f"{my_path}/beam_convolved_actfoot_dlensed_tsz_map.fits" # + high resolution tsz map
# b_l_tsz_map = f"{my_path}/beam_convolved_actfoot_dlensed_tsz2048_map.fits" # + low resolution tsz map 
b_l_ksz_cib_map = f"{my_path}/beam_convolved_actfoot_dlensed_ksz_cib_map.fits"
b_l_tsz_ksz_cib_map = f"{my_path}/beam_convolved_actfoot_dlensed_tsz_ksz_cib_map.fits"
...
try:
    ...
    try:  
        ...
        # print(" ::: reprojected maps are saved!") 
...
print(" ::: adding white noise of %.1f on maps" %nlevel)
white_noise = maps.white_noise(lmap.shape, lmap.wcs, noise_muK_arcmin=nlevel)
lmap += white_noise
l_tsz_map += white_noise
l_ksz_cib_map += white_noise
l_tsz_ksz_cib_map += + white_noise

# test using true tSZ model that is pasted into the cmb map when creating the sims
if true_map == True: 
    print("reading in simMap_f150.fits for cmb+tsz map")
    l_tsz_map = enmap.read_map(f"{test_path}/simMap_f150.fits")
    print(np.shape(l_tsz_map))
...
for task in my_tasks:
    ...
    # if test == True:
    #     io.plot_img(grad[100:140,100:140], f"{save_dir}/{save_name}_grad_zoomed_%d.png" %j) 
    ...
    if inpainting == True:
        ...
        """
        rmin = 4 * utils.arcmin # 4 or 6 or 8
        ...
        inp_stamp = maps.filter_map(np.nan_to_num(tapered_grad/taper), ymask) # dividing by taper returns nans 
        s.add_to_stack("inp_stamp_st", inp_stamp)  
        # if test == True:
        #     io.plot_img(np.nan_to_num(tapered_grad/taper)[100:140,100:140], f"{save_dir}/{save_name}_inp_zoomed_%d.png" %j)   
