...
pathname="/home3/nehajo/projects/CMASS-cmbhalolensing/sims/agora/agora_ell_test/agora_ell_test"
mf_pathname="/home3/nehajo/projects/CMASS-cmbhalolensing/sims/agora_highlmin/agora_cmb_90/agora_cmb_90"
...
temp_stamp = enmap.read_map(f"{pathname}_template.fits")
shape, wcs = temp_stamp.shape, temp_stamp.wcs
modlmap = enmap.modlmap(shape, wcs)
kmask = enmap.read_map(f"{pathname}_kmask.fits")
mf = enmap.enmap(np.load(f"{mf_pathname}_mf_1rkappa.npy"), wcs)
mf_err = np.loadtxt(f"{mf_pathname}_mf_1rkappa_errs.txt")
...
np.savetxt(f"{pathname}_post_1binned_rlkappa.txt", Clt1d_mfsub)
pl = Plotter(xyscale='linlin', xlabel='$L$', ylabel='$C_\ell$')
pl.add_err(cents, Clt1d_mfsub, Clt_errs, ls="-", label="cmb recon x template \nmfsub", color="tab:blue")
pl.add_err(cents, Ckt1d, Ckt_errs, ls="-", color="black", label="true x template")
pl._ax.axhline(y=0)
pl.add(cents, Cmt1d, ls="--", color="tab:blue")
