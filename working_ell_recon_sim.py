...
parser.add_argument(
    "--template-mass", type=float, default=2., help="Mass (e13 Msun) used to make template for ell-space correlations."
)
...
ell_bin_edges = np.arange(klmin, klmax, 200)
ell_centers = (ell_bin_edges[1:] + ell_bin_edges[:-1]) / 2.0
...
for task in my_tasks:
    ...
    # stack kappa in Fourier space
    Clt_stamp= (rkmap*np.conjugate(temp_ellstamp)).real
    Clt1d = bin(Clt_stamp, modlmap, ell_bin_edges)
    Clt1ds.append(Clt1d.copy())
    ...
    if j == 0:
        ...
        if args.debug:
            ...
            lstamps = kappa.copy()
            kellstamps = Ckt_stamp.copy()

    else:
        hstamps = hstamps + hres_stamp
        gstamps = gstamps + grad_stamp
        ellstamps = ellstamps + Clt_stamp
        if args.debug:
            ...
            kellstamps = kellstamps + Ckt_stamp
...
if not args.debug:
    ...
    ellstamps = np.asarray(ellstamps)
    ellmap = utils.reduce(ellstamps, comm, root=0, op=mpi.MPI.SUM)

    lstamps = np.asarray(lstamps)
    lmap = utils.reduce(lstamps, comm, root=0, op=mpi.MPI.SUM)

    kellstamps = np.asarray(kellstamps)
    kellmap = utils.reduce(kellstamps, comm, root=0, op=mpi.MPI.SUM)
...
if rank==0:
    ...
    if not args.debug:
        ...
        print(k1ds.shape[0])
        print(tk1ds.shape[0])
        print(Clt1ds.shape[0])
        print(Ckt1ds.shape[0])
        assert k1ds.shape[0] == Nobj[0]
        assert tk1ds.shape[0] == Nobj[0]
        assert Clt1ds.shape[0] == Nobj[0]
        assert Ckt1ds.shape[0] == Nobj[0]
    ...
    if not args.debug:
        ...
        ellmap = enmap.enmap(ellmap, wcs)
        lmap = enmap.enmap(lmap, wcs)
        kellmap = enmap.enmap(kellmap, wcs)
        kmap /= Nobj
        ellmap /= Nobj
        lmap /= Nobj
        kellmap /= Nobj

        # fft-shift fourier space map for plotting
        ellmap_shifted = enmap.fftshift(ellmap)
        kellmap_shifted = enmap.fftshift(kellmap)
        ...
        modrmap = np.rad2deg(modrmap)*60.
        modlmap = ellmap.modlmap() 

        kbinned = bin(kmap_zoom, modrmap, bin_edges)
        lbinned = bin(lmap_zoom, modrmap, bin_edges)
        ellbinned = bin(ellmap, modlmap, ell_bin_edges)
        # print(kellmap[0])
        kellbinned = bin(kellmap, modlmap, ell_bin_edges)


        io.plot_img(kmap, f"{save_dir}/{save_name}_1tkappa.png")   
        io.plot_img(lmap, f"{save_dir}/{save_name}_1rkappa.png")
        io.plot_img(ellmap_shifted, f"{save_dir}/{save_name}_1rlkappa.png")       
        io.plot_img(kellmap_shifted, f"{save_dir}/{save_name}_1tlkappa.png")              
        ...
        np.save(f"{save_dir}/{save_name}_1rlkappa.npy", ellmap)     
        np.save(f"{save_dir}/{save_name}_1tlkappa.npy", kellmap)             
        ...
        io.save_cols(f"{save_dir}/{save_name}_1binned_rlkappa_from2d.txt", (ell_centers, ellbinned))
        io.save_cols(f"{save_dir}/{save_name}_1binned_tlkappa_from2d.txt", (ell_centers, kellbinned))
        ...
        lbinned = Clt1ds.mean(axis=0)
        lerrs, lcovm = profiles.errors(Clt1ds, Nobj=Clt1ds.shape[0])
        lcorr = profiles.correlation_matrix(lcovm)

        tlbinned = Ckt1ds.mean(axis=0)
        tlerrs, tlcovm = profiles.errors(Ckt1ds, Nobj=Ckt1ds.shape[0])
        tlcorr = profiles.correlation_matrix(tlcovm)

        np.savetxt(f"{save_dir}/{save_name}_1tkappa_errs.txt", terrs)               
        np.savetxt(f"{save_dir}/{save_name}_1rkappa_errs.txt", errs)    
        np.savetxt(f"{save_dir}/{save_name}_1rlkappa_errs.txt", lerrs)
        np.savetxt(f"{save_dir}/{save_name}_1tlkappa_errs.txt", tlerrs)      
        np.savetxt(f"{save_dir}/{save_name}_1tkappa_covm.txt", tcovm)
        np.savetxt(f"{save_dir}/{save_name}_1rkappa_covm.txt", covm)
        np.savetxt(f"{save_dir}/{save_name}_1rlkappa_covm.txt", lcovm)
        np.savetxt(f"{save_dir}/{save_name}_1tlkappa_covm.txt", tlcovm)
        np.save(f"{save_dir}/{save_name}_1tkappa_corr.npy", tcorr)
        np.save(f"{save_dir}/{save_name}_1rkappa_corr.npy", corr) 
        np.save(f"{save_dir}/{save_name}_1rlkappa_corr.npy", lcorr)
        np.save(f"{save_dir}/{save_name}_1tlkappa_corr.npy", tlcorr)
        ...
        io.save_cols(f"{save_dir}/{save_name}_1binned_rlkappa.txt", (ell_centers, lbinned))
        io.save_cols(f"{save_dir}/{save_name}_1binned_tlkappa.txt", (ell_centers, tlbinned))
